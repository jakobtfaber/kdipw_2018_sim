...
def gen_pl_phase_screen_2d(si, phiF, rF, inner, outer, dx, dy, xwidth, ywidth, apply_inner=False,  apply_outer=False, normfres=True, debug=False):
    ...
    if nqx != nx:
        nqx = 3072
        print "Forcing nqx = nx = ", nx, nqx
    #nqx = nx
    if nqy != ny:
        nqy = 3072
        print "Forcing nqy = ny = ", ny, nqy
    #nqy = ny
    ...
    print "min qxvec", np.min(qxvec)
    print "max qxvec", np.max(qxvec)
    print "min qyvec", np.min(qyvec)
    print "max qyvec", np.max(qyvec)
    ...
    qxi, qyj = make_2d(qxvec, qyvec)

    qsq = qxi**2 + qyj**2
    qshape = (qout**2 + qsq)**(-si/4.) * exp(-qsq/(2.*qmax**2))
    qshape_rolloff = exp(-qsq / (2.*qin**2))
    if apply_outer:
        qshape *= exp(-qout**2 / (2.*qsq))
