...
def plot_intensity_slice():
    ...
    title(r'$\rm Slice \ Through \ Spatial \ Intensity \ Pattern \  from \  Screen \ \ \ DISS, Lens = (%d, %d)$'%(int(do_diffraction), int(do_gaussian)), fontsize=13)
    annotate(r'$\rm {\bf Diffraction}: \ \phi_F = %5.2f\ rad \ \ \ {\bf Gaussian \ Lens}: \ \phi_G = %5.2f \ rad $'%(phiF_d*float(do_diffraction), phiblob*float(do_gaussian)) , xy=(0.5, 0.95), xycoords='figure fraction', ha='center', fontsize=12)
    show()
...
def plot_wavefield_phase():
    ...
    title(r'$\rm Slice \ Through \ Phase \ of\  Wavefield\ Pattern  \ \ \ DISS, Lens = (%d, %d)$'%(int(do_diffraction), int(do_gaussian)), fontsize=13)
    annotate(r'$\rm {\bf Diffraction}: \ \phi_F = %5.2f\ rad \ \ \ {\bf Gaussian \ Lens}: \ \phi_G = %5.2f \ rad $'%(phiF_d*float(do_diffraction), phiblob*float(do_gaussian)) , xy=(0.5, 0.95), xycoords='figure fraction', ha='center', fontsize=12)
    show()
